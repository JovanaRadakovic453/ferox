...
> model, energetski motor, konvencije, deploy. Svakodnevni "kako radim" kontekst
> je u `CLAUDE.md`.
...
> ⚠️ **Ovo NIJE Next.js koji znaš** (vidi vrh ovog fajla). Next 16 ima breaking
...
npm run typecheck      # tsc --noEmit (ne radi TLS → ne pada od cert buga)
```
Node 20+.
...
Šablon je `.env.example` (`cp .env.example .env.local`). Obavezne javne varijable
(`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`) validiraju se pri startu u
`lib/env.ts` (jasna greška ako fale). Server-only tajne su opcione/feature-gated.
```
...
lib/
  ...
  env.ts               — zod validacija OBAVEZNIH javnih env var pri startu (jasna greška ako fale)
  anthropic.ts         — Anthropic klijent (SAMO server-side)
  config.ts            — centralni TUNABLES: AI (model/max_tokens/capovi), DEFAULTS (voda/pomodoro/podsetnik/streak/history/insights), RATE_LIMITS (po ruti), APP (meta/url)
  ...
  api.ts               — apiOk / apiError / ERR (standardni { error:{code,message} }; ERR.rateLimited=429)
  validation.ts        — zod sheme (sve rute) + zStrictDate/zTime; enum-i (TASK_TYPES/PRIORITIES) se uvoze iz types/ferox.ts
...
types/ferox.ts         — Svi tipovi + LABELS + enum const-nizovi (TASK_TYPES/PRIORITIES → izveden union; jedan izvor istine, mora se poklapati sa schema.sql)
