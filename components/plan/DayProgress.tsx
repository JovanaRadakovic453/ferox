...
export default function DayProgress({ blocks }: { blocks: PlanBlock[] }) {
  ...
  if (active.length === 0) {
    return <div className="h-1 rounded-full" style={{ background: 'var(--surface2)' }} />
  }
  return (
    <div className="flex items-center gap-1.5" aria-hidden>
      {active.map(b => {
        ...
        return (
          <div key={b.label} className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: 'var(--surface2)', boxShadow: 'inset 0 1px 2px rgba(26,23,20,0.07)' }}>
            <div
